---
layout: post
title: "[AI 개발] 1부 LLM 사용기 - QA 엔지니어로 일하며 AI 도구로 테스트 케이스 관리툴 만든 경험담"
...
Perplexity나 ChatGPT 같은 AI한테 코드 조금씩 받아서, 복사 붙여넣기 하면서 하나씩 만들어 나가는 방식이었죠.

그런데 몇 번 반복하다 보니, ‘이거… 효율이 너무 떨어지는데?’ 싶더라고요.

그래서 조금씩 도구를 바꿔봤습니다.
파일 업로드로 전체 구조를 이해시키는 방법을 써보고, 직접 IDE에서 수정까지 해주는 에디터(Cursor)로 옮겨가고, 테스트나 요구사항 작성까지 해주는 KIRO도 써봤죠.
그 과정에서 Claude Code, JIRA, 그리고 Playwright까지 붙이면서, 단순한 “복붙 개발”에서 “프로젝트 관리 + 자동화 테스트까지 하는 개발”로 흐름이 바뀌었습니다.

결국 복잡한 기능 구현보다 더 힘들었던 건 ‘어떤 환경이 가장 편하고, 나한테 맞는가’ 찾는 일이었어요.
이번 글은 제가 테스트 케이스 관리툴을 만들면서 거쳐 간 그 도구들과 작업 방식의 변화를 시간 순서대로 정리한 기록입니다.
혹시 AI로 개발을 시작하려는 분이라면, 어디서부터 시작해도 좋을지 감이 잡히실 거예요.
