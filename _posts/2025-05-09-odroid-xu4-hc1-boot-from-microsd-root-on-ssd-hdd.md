...
안녕하세요! 오늘은 Odroid XU4, HC1 같은 구형 보드에서
microSD는 부트용, SSD/HDD는 루트 파일시스템으로 쓰는 방법을 정리해볼게요.
...
1. **SSD/HDD 연결**
    - USB-SATA 어댑터(또는 HC1은 SATA)에 SSD/HDD를 연결해요.
2. **디스크 인식 확인**
```bash
lsblk
```
...
```
    ...
    - `d`로 기존 파티션 삭제, `n`으로 새로 만들기, `w`로 저장
    4. **포맷(ext4 추천!)**
```bash
...
- UUID 대신 `/dev/sda1`로 해도 되지만, UUID가 더 안전!
...
- microSD는 이제 부트로더/커널만 담당하고,
실질적인 시스템 구동은 SSD/HDD에서 이뤄져요!
- 속도도 빨라지고, SD카드 수명 걱정도 끝!
- SSD/HDD로 옮기면 정말 쾌적해집니다. 강추👍

---

궁금한 점이나 막히는 부분 있으면 댓글로 남겨주세요!
모두 쾌적한 Odroid 라이프 되시길 바라요 😄
...
아래처럼 더 자연스럽고 읽기 쉽게, 블로그 감성에 맞게 다듬어봤어요!
중간중간 소제목과 강조, 문장 흐름을 부드럽게 손봤습니다.
